...
import { Plus, Trash2, Users, Megaphone, BookOpen, LayoutGrid, X, Loader2, Settings2, Edit2, Check, AlertCircle, UserCircle2, CheckCircle2, Clock, ArrowLeft, Search, History, ShieldCheck, Save, FileText, ClipboardList } from 'lucide-react';
...
export default function AdminDashboard({ profile, isSuperAdmin = false }: AdminDashboardProps) {
  ...
  useEffect(() => {
    ...
    const unsubNotes = onSnapshot(getSectionCollection(profile, 'notes'), (snapshot) => {
      setResourceCounts((prev) => ({ ...prev, notes: snapshot.docs.filter((item) => !item.data().deleteRequested).length }));
    });

    const unsubPreviousQuestions = onSnapshot(getSectionCollection(profile, 'previousQuestions'), (snapshot) => {
  ...
  const handleUpdateAssignment = async () => {
    ...
    if (!editingAssignment.title.trim() || !editingAssignment.course.trim() || !editingAssignment.deadline) return;

    setIsUpdating(true);
    try {
      await updateDoc(getSectionDoc(profile, 'assignments', editingAssignment.id), {
        title: editingAssignment.title.trim(),
        course: editingAssignment.course.trim(),
        deadline: editingAssignment.deadline,
        type: editingAssignment.type,
        syllabus: editingAssignment.syllabus || '',
        urgency: calculateUrgency(editingAssignment.deadline, editingAssignment.completed_hours || 0, editingAssignment.total_hours || 1),
      });
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
        {[
          { icon: Users, label: 'Section Management', value: users.filter(u => u.role === 'student').length, color: 'neutral', tab: 'students' as const },
          { icon: BookOpen, label: 'Course Management', value: courses.length, color: 'blue', tab: 'courses' as const },
          { icon: Clock, label: 'Task Management', value: allAssignments.filter(a => !isTaskDone(a)).length, color: 'neutral', tab: 'assignments' as const },
          { icon: Megaphone, label: 'Notice', value: announcements.length, color: 'amber', tab: 'announcements' as const },
          { icon: CheckCircle2, label: 'Done', value: allAssignments.filter(a => isTaskDone(a)).length, color: 'green', valueColor: 'text-green-600', tab: 'history' as const },
          { icon: FileText, label: 'Notes', value: resourceCounts.notes, color: 'violet', tab: 'notes' as const },
          { icon: ClipboardList, label: 'Previous Question', value: resourceCounts.previousQuestions, color: 'cyan', tab: 'previousQuestions' as const },
        ].map((stat, idx) => (
          <motion.div 
            key={stat.label}
            variants={{
          ...
          >
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 ${
              ...
              stat.color === 'cyan' ? 'bg-cyan-50 dark:bg-cyan-900/20 text-cyan-500' :
      ...
      {activeTab === 'overview' && (
        <div className="bg-white dark:bg-neutral-900 p-8 rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-sm text-center">
          <p className="text-sm font-semibold text-neutral-700 dark:text-neutral-200">Select a dashboard card to open that section.</p>
      ...
      {activeTab !== 'overview' && (<><AnimatePresence mode="wait">
        ...
        {activeTab === 'courses' && (
          ...
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-4 shadow-sm">
              <div>
                <h3 className="text-xl font-black text-neutral-900 dark:text-neutral-50 tracking-tight">Course Management</h3>
                <p className="text-[10px] text-neutral-400 dark:text-neutral-500 font-bold uppercase tracking-[0.2em] mt-1">Create courses and manage your section course list</p>
              </div>
              <div className="flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() => setCourseManagementMode('create')}
                  className={`px-5 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${courseManagementMode === 'create' ? 'bg-neutral-900 text-white dark:bg-neutral-50 dark:text-neutral-900' : 'bg-neutral-100 text-neutral-500 hover:text-neutral-900 dark:bg-neutral-800 dark:text-neutral-400 dark:hover:text-neutral-50'}`}
                >
                  Create Course
                </button>
                <button
                  type="button"
                  onClick={() => setCourseManagementMode('list')}
                  className={`px-5 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${courseManagementMode === 'list' ? 'bg-neutral-900 text-white dark:bg-neutral-50 dark:text-neutral-900' : 'bg-neutral-100 text-neutral-500 hover:text-neutral-900 dark:bg-neutral-800 dark:text-neutral-400 dark:hover:text-neutral-50'}`}
                >
                  Course List
                </button>
              </div>
            ...
            {courseManagementMode === 'create' ? (
              <div className="max-w-xl bg-white dark:bg-neutral-900 p-6 rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-sm">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="text-lg font-semibold text-neutral-900 dark:text-neutral-50 tracking-tight">Create New Course</h3>
                </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between px-1">
                  <h3 className="text-xl font-semibold text-neutral-900 dark:text-neutral-50 tracking-tight">Course List / Edit / Delete</h3>
                      ...
                      {editingCourse?.id === course.id ? (
                        <div className="flex-1 flex items-center gap-2">
                          <input
                            ...
                            autoFocus
                          />
                          <button onClick={() => setShowConfirmUpdate(true)} className="p-2 text-green-600 dark:text-green-500 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-lg transition-all">
                            <Check className="w-4 h-4" />
                          </button>
                          <button onClick={() => setEditingCourse(null)} className="p-2 text-neutral-400 dark:text-neutral-500 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-lg transition-all">
                            <X className="w-4 h-4" />
                          </button>
                        </div>
                      ) : (
                        <>
                          ...
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => setEditingCourse(course)} className="p-2 text-neutral-400 dark:text-neutral-500 hover:text-neutral-900 dark:hover:text-neutral-50 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-lg transition-all">
                              <Edit2 className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => { setDocToDelete({ col: 'courses', id: course.id! }); setShowConfirmDelete(true); }} className="p-2 text-red-400 dark:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-all">
            ...
            <AnimatePresence>
              {showConfirmUpdate && (
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowConfirmUpdate(false)} className="absolute inset-0 bg-neutral-900/40 dark:bg-neutral-900/60 backdrop-blur-sm" />
                  <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-sm bg-white dark:bg-neutral-900 rounded-[2.5rem] shadow-2xl p-8 text-center border border-transparent dark:border-neutral-800">
                    ...
                    <div className="flex gap-3">
                      <button onClick={() => setShowConfirmUpdate(false)} className="flex-1 px-4 py-3 bg-neutral-100 dark:bg-neutral-800 text-neutral-900 dark:text-neutral-50 rounded-xl font-bold hover:bg-neutral-200 dark:hover:bg-neutral-700 transition-all text-xs uppercase tracking-widest">Cancel</button>
                      <button onClick={handleUpdateCourse} disabled={isUpdating} className="flex-1 px-4 py-3 bg-neutral-900 dark:bg-neutral-50 text-white dark:text-neutral-900 rounded-xl font-bold hover:bg-neutral-800 dark:hover:bg-neutral-200 transition-all disabled:opacity-50 flex items-center justify-center gap-2 text-xs uppercase tracking-widest">
        ...
        {activeTab === 'previousQuestions' && (
          <motion.div key="previousQuestions" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
        ...
        {activeTab === 'assignments' && (
          ...
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-4 shadow-sm">
              <div>
                <h3 className="text-xl font-black text-neutral-900 dark:text-neutral-50 tracking-tight">Task Management</h3>
                <p className="text-[10px] text-neutral-400 dark:text-neutral-500 font-bold uppercase tracking-[0.2em] mt-1">Create tasks and manage task list with sorting</p>
              </div>
              <div className="flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() => setTaskManagementMode('create')}
                  className={`px-5 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${taskManagementMode === 'create' ? 'bg-neutral-900 text-white dark:bg-neutral-50 dark:text-neutral-900' : 'bg-neutral-100 text-neutral-500 hover:text-neutral-900 dark:bg-neutral-800 dark:text-neutral-400 dark:hover:text-neutral-50'}`}
                >
                  Create Task
                </button>
                <button
                  type="button"
                  onClick={() => setTaskManagementMode('list')}
                  className={`px-5 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${taskManagementMode === 'list' ? 'bg-neutral-900 text-white dark:bg-neutral-50 dark:text-neutral-900' : 'bg-neutral-100 text-neutral-500 hover:text-neutral-900 dark:bg-neutral-800 dark:text-neutral-400 dark:hover:text-neutral-50'}`}
                >
                  Task List
                </button>
              </div>
            ...
            {taskManagementMode === 'create' ? (
              <div className="max-w-2xl bg-white dark:bg-neutral-900 p-6 rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-sm">
                <div className="flex items-center gap-3 mb-6">
                ...
                <form onSubmit={handleAddAssignment} className="space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input required list="course-list" type="text" value={newAssignment.course} onChange={e => setNewAssignment({...newAssignment, course: e.target.value})} placeholder="Select or type course" className="w-full pl-11 pr-4 py-3 bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-neutral-900/5 focus:border-neutral-900/20 transition-all text-sm font-medium text-neutral-900 dark:text-neutral-50" />
                      <datalist id="course-list">
                        {courses.map(s => <option key={s.id} value={s.name} />)}
                      </datalist>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      {['Quiz', 'Assignment', 'Presentation', 'Lab'].map((t) => (
                        <button key={t} type="button" onClick={() => setNewAssignment({ ...newAssignment, type: t as any })} className={`py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-wider border transition-all ${newAssignment.type === t ? 'bg-neutral-900 dark:bg-neutral-50 text-white dark:text-neutral-900 border-neutral-900 dark:border-neutral-50 shadow-md' : 'bg-white dark:bg-neutral-900 text-neutral-500 dark:text-neutral-400 border-neutral-200 dark:border-neutral-800 hover:border-neutral-300 dark:hover:border-neutral-700'}`}>{t}</button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea value={newAssignment.syllabus} onChange={e => setNewAssignment({...newAssignment, syllabus: e.target.value})} placeholder="Enter topics or instructions..." rows={3} className="w-full px-4 py-3 bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-neutral-900/5 focus:border-neutral-900/20 transition-all text-sm font-medium text-neutral-900 dark:text-neutral-50 resize-none" />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input required type="datetime-local" value={newAssignment.deadline} onChange={e => setNewAssignment({...newAssignment, deadline: e.target.value})} className="w-full pl-11 pr-4 py-3 bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-neutral-900/5 focus:border-neutral-900/20 transition-all text-sm font-medium text-neutral-900 dark:text-neutral-50 [color-scheme:light] dark:[color-scheme:dark]" />
                    </div>
                  </div>

                  <button type="submit" disabled={isUpdating} className="w-full bg-neutral-900 dark:bg-neutral-50 text-white dark:text-neutral-900 py-4 rounded-2xl font-bold hover:bg-neutral-800 dark:hover:bg-neutral-200 transition-all shadow-xl shadow-neutral-200 dark:shadow-none active:scale-[0.98] flex items-center justify-center gap-3 text-xs uppercase tracking-widest mt-4">
                    {isUpdating ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Plus className="w-4 h-4" /> Create & Assign Task</>}
                  </button>
                </form>
              </div>
            ) : (
              <div className="space-y-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-2">
                  <div>
                    <h3 className="text-2xl font-bold text-neutral-900 dark:text-neutral-50 tracking-tight">Task List / Edit / Delete</h3>
                    <p className="text-[11px] text-neutral-400 dark:text-neutral-500 font-semibold uppercase tracking-widest mt-1">Sort by time left or creation order</p>
                  </div>
                  <div className="flex flex-wrap gap-2 bg-white dark:bg-neutral-900 p-2 rounded-2xl border border-neutral-200 dark:border-neutral-800">
                    {[
                      { key: 'timeLeft', label: 'Time Left' },
                      { key: 'newest', label: 'Newest Added' },
                      { key: 'oldest', label: 'Oldest Added' },
                    ].map((item) => (
                      <button key={item.key} type="button" onClick={() => setTaskSortMode(item.key as any)} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${taskSortMode === item.key ? 'bg-neutral-900 text-white dark:bg-neutral-50 dark:text-neutral-900' : 'text-neutral-500 hover:text-neutral-900 dark:text-neutral-400 dark:hover:text-neutral-50'}`}>{item.label}</button>
                    ))}
                  </div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse min-w-[900px]">
                      <thead>
                        <tr className="bg-neutral-50/50 dark:bg-neutral-800/50 border-b border-neutral-100 dark:border-neutral-800">
                          <th className="px-6 py-5 text-[10px] font-bold text-neutral-400 uppercase tracking-widest">Task Details</th>
                          <th className="px-6 py-5 text-[10px] font-bold text-neutral-400 uppercase tracking-widest">Assigned Student</th>
                          <th className="px-6 py-5 text-[10px] font-bold text-neutral-400 uppercase tracking-widest">Progress</th>
                          <th className="px-6 py-5 text-[10px] font-bold text-neutral-400 uppercase tracking-widest">Urgency</th>
                          <th className="px-6 py-5 text-[10px] font-bold text-neutral-400 uppercase tracking-widest">Deadline</th>
                      ...
                      <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800">
                        {allAssignments.filter(a => !isTaskDone(a)).length === 0 ? (
                          ...
                          sortAssignments(allAssignments.filter(a => !isTaskDone(a))).map(assignment => {
                            ...
                            return (
                              <tr key={assignment.id} className="hover:bg-neutral-50/50 dark:hover:bg-neutral-800/50 transition-colors group align-top">
                                <td className="px-6 py-6">
                                  {isEditing ? (
                                    <div className="space-y-2 min-w-[260px]">
                                      <input value={editingAssignment.title} onChange={(e) => setEditingAssignment({ ...editingAssignment, title: e.target.value })} className="w-full px-3 py-2 rounded-xl bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 text-sm font-bold text-neutral-900 dark:text-neutral-50" />
                                      <input list="edit-course-list" value={editingAssignment.course} onChange={(e) => setEditingAssignment({ ...editingAssignment, course: e.target.value })} className="w-full px-3 py-2 rounded-xl bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 text-xs font-bold text-neutral-900 dark:text-neutral-50" />
                                      <datalist id="edit-course-list">{courses.map(c => <option key={c.id} value={c.name} />)}</datalist>
                                      <select value={editingAssignment.type} onChange={(e) => setEditingAssignment({ ...editingAssignment, type: e.target.value as Assignment['type'] })} className="w-full px-3 py-2 rounded-xl bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 text-xs font-bold text-neutral-900 dark:text-neutral-50">
                                        {['Quiz', 'Assignment', 'Presentation', 'Lab'].map(type => <option key={type} value={type}>{type}</option>)}
                                      </select>
                                      <textarea value={editingAssignment.syllabus || ''} onChange={(e) => setEditingAssignment({ ...editingAssignment, syllabus: e.target.value })} rows={2} className="w-full px-3 py-2 rounded-xl bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 text-xs font-medium text-neutral-900 dark:text-neutral-50" placeholder="Syllabus / details" />
                                    </div>
                                  ) : (
                                ...
                                <td className="px-6 py-6">
                                  <div className="flex items-center gap-3">
                                    <div className="w-9 h-9 bg-neutral-100 dark:bg-neutral-800 rounded-xl flex items-center justify-center text-neutral-400 transition-transform group-hover:scale-110">
                                      {student?.photoURL ? <img src={student.photoURL} alt="" className="w-full h-full object-cover rounded-xl" referrerPolicy="no-referrer" /> : <UserCircle2 className="w-5 h-5" />}
                                    </div>
                                    <div>
                                ...
                                <td className="px-6 py-6">
                                  <div className="space-y-2 max-w-[120px]">
                                    ...
                                    <div className="h-1.5 w-full bg-neutral-100 dark:bg-neutral-800 rounded-full overflow-hidden">
                                      <motion.div initial={{ width: 0 }} animate={{ width: `${(assignment.completed_hours / (assignment.total_hours || 1)) * 100}%` }} className={`h-full ${urgency === 'done' ? 'bg-emerald-500' : urgency === 'urgent' ? 'bg-red-500' : urgency === 'medium' ? 'bg-amber-500' : 'bg-blue-500'}`} />
                                    </div>
                                  </div>
                                </td>
                                <td className="px-6 py-6">
                                  <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest ${urgency === 'done' ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400' : urgency === 'urgent' ? 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400' : urgency === 'medium' ? 'bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400' : 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400'}`}>
                                    {urgency}
                                  </span>
                                </td>
                                <td className="px-6 py-6 whitespace-nowrap">
                                  {isEditing ? (
                                    <input type="datetime-local" value={editingAssignment.deadline} onChange={(e) => setEditingAssignment({ ...editingAssignment, deadline: e.target.value })} className="px-3 py-2 rounded-xl bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 text-xs font-bold text-neutral-900 dark:text-neutral-50 [color-scheme:light] dark:[color-scheme:dark]" />
                                  ) : (
                                ...
                                <td className="px-6 py-6 text-right">
                                  <div className="flex justify-end gap-2">
                                    {isEditing ? (
                                      <>
                                        <button onClick={handleUpdateAssignment} disabled={isUpdating} className="p-2.5 text-green-600 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-xl transition-all">
                                          {isUpdating ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                                        </button>
                                        <button onClick={() => setEditingAssignment(null)} className="p-2.5 text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-xl transition-all"><X className="w-4 h-4" /></button>
                                      </>
                                    ) : (
                                      <>
                                        <button onClick={() => setEditingAssignment({ ...assignment })} className="p-2.5 text-neutral-400 hover:text-neutral-900 dark:hover:text-neutral-50 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-xl transition-all"><Edit2 className="w-4 h-4" /></button>
                                        <button onClick={() => { setDocToDelete({ col: 'assignments', id: assignment.id! }); setShowConfirmDelete(true); }} className="p-2.5 text-neutral-300 hover:text-red-600 dark:hover:text-red-400 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-xl transition-all"><Trash2 className="w-4 h-4" /></button>
                                      </>
                                    )}
                                  </div>
                                </td>
        ...
        {activeTab === 'students' && (
          ...
          >
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-4 shadow-sm">
              <div>
                <h3 className="text-xl font-black text-neutral-900 dark:text-neutral-50 tracking-tight">Section Management</h3>
                <p className="text-[10px] text-neutral-400 dark:text-neutral-500 font-bold uppercase tracking-[0.2em] mt-1">Manage enrollment key and student directory</p>
              </div>
              <div className="flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() => setSectionManagementMode('enrollment')}
                  className={`px-5 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${sectionManagementMode === 'enrollment' ? 'bg-neutral-900 text-white dark:bg-neutral-50 dark:text-neutral-900' : 'bg-neutral-100 text-neutral-500 hover:text-neutral-900 dark:bg-neutral-800 dark:text-neutral-400 dark:hover:text-neutral-50'}`}
                >
                  Enrollment Key
                </button>
                <button
                  type="button"
                  onClick={() => setSectionManagementMode('students')}
                  className={`px-5 py-3 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${sectionManagementMode === 'students' ? 'bg-neutral-900 text-white dark:bg-neutral-50 dark:text-neutral-900' : 'bg-neutral-100 text-neutral-500 hover:text-neutral-900 dark:bg-neutral-800 dark:text-neutral-400 dark:hover:text-neutral-50'}`}
                >
                  Students
                </button>
              </div>
            ...
            {sectionManagementMode === 'enrollment' ? (
                ...
                <form onSubmit={handleUpdateSectionKey} className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      onChange={(e) => setSectionJoinCode(e.target.value)}
                      placeholder="e.g. CSE70C"
                      className="w-full px-4 py-3 bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl text-sm font-bold text-neutral-900 dark:text-neutral-50 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                    />
                  </div>
                  <button
                    type="submit"
                    disabled={isProcessing === 'section-key'}
                    className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl bg-neutral-900 text-white dark:bg-neutral-50 dark:text-neutral-900 text-xs font-black uppercase tracking-widest disabled:opacity-50"
                  >
              ...
              <div className="space-y-10">
                ...
                <div className="relative w-full md:max-w-sm">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-3 rounded-2xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 text-sm font-semibold text-neutral-900 dark:text-neutral-50 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                  />
                </div>
